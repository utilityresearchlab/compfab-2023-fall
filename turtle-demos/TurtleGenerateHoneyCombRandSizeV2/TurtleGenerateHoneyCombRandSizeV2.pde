...
void draw() {
  ...
  for (int i = 0; i < rowCount; i += 1) {
    ...
    t.setHeading(30);
    float randSize = sideLength;
    for (int j = 0; j < hexCount; j += 1) {
      randSize = random(1, sideLength);
      // Top row
      drawTopHalfHex(randSize);
      t.left(120);
      t.forward(randSize);
      t.left(60);
    }
...
void drawBottomHalfHex(float l) {
  t.setHeading(150);
  for (int i = 0; i < 3; i += 1) {
    t.forward(l);
    t.left(60);
